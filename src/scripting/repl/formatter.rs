...
use crate::scripting::object::{BuiltinTableKind, CeChild, MaterializedCE, Value};
use crate::scripting::unparser::{unparse_component, unparse_expression};
...
pub fn format_repl_value(value: &Value, world: &World) -> Result<String, String> {
    match value {
        ...
        Value::TransformWorld { id } => Ok(format!("<Transform.world:{id:?}>")),
...
fn materialized_to_ast(ce: &MaterializedCE) -> Result<ComponentExpression, String> {
    ...
    for (method, args) in &ce.calls {
        body.push(Statement::Expression(Expression::Call(
            crate::scripting::ast::CallExpression {

// data-viz-http-unbounded.mms
//
// POST {"value": 12} to http://127.0.0.1:7000/ to append one bar.
// Requires: JSON.parse(text) and mutable array push.
...
let chart = T.position(-2.8, 1.8, 0.0).scale(0.12, 0.12, 0.12) {
    LayoutRoot {
        ...
        T {
            ...
            Text { "HTTP bar chart — unbounded" C.rgba(0.88, 0.94, 1.0, 1.0) }
            T {
